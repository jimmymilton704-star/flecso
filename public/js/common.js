...
(function () {
    ...
    const statusBadge = (status) => {
        ...
        const map = {
            ...
            "in-transit": ["badge--success", "In Transit"],
            completed: ["badge--success", "Completed"],
            available: ["badge--info", "Available"],
            loading: ["badge--info", "Loading"],
            idle: ["badge--neutral", "Idle"],
            "off-duty": ["badge--neutral", "Off Duty"],
            inactive: ["badge--neutral", "Inactive"],
            maintenance: ["badge--warn", "Maintenance"],
            delayed: ["badge--warn", "Delayed"],
            "on-leave": ["badge--warn", "On Leave"],
            expiring: ["badge--warn", "Expiring"],
    ...
    const selectField = (label, options) =>
        `<div class="field"><label>${label}</label><select>${options
            .split(",")
    ...
    function openDrawer(title, subtitle, bodyHtml) {
        ...
        $$(".delivery-opt").forEach((o) =>
            o.addEventListener("click", () => {
                $$(".delivery-opt").forEach((x) =>
    ...
    $("#drawerSubmit")?.addEventListener("click", () => {
        closeAll();
    ...
    $("#qrPrint")?.addEventListener("click", () => {
        ...
        w.document.write(
            `<html><head><title>Print QR</title></head><body style="display:grid;place-items:center;height:100vh;font-family:Inter"><div style="text-align:center"><h2>${$("#qrTitle").textContent}</h2><img src="${dataUrl}" style="width:280px;height:280px"><p>${$("#qrData").textContent}</p></div><script>window.onload=()=>window.print()<\/script></body></html>`,
        );
        w.document.close();
    });
    ...
    const notifications = [
        {
            ...
            icon: "bell",
            tone: "danger",
            title: "SOS alert from Luca Romano",
            sub: "A1 Milano-Napoli · KM 412 — response team dispatched",
            time: "2m",
            unread: true,
        },
        {
            ...
            icon: "spark",
            tone: "warn",
            title: "Maintenance due for MXL-108",
            sub: "Scheduled service window opens in 3 days",
            time: "18m",
            unread: true,
        },
        {
            ...
            sub: "Francesca Rizzo's license expires 2026-05-14",
            time: "1h",
            unread: true,
        },
        {
            ...
            icon: "check",
            tone: "success",
            title: "Trip TR-20476 completed",
            sub: "Verona → Vienna delivered on-time",
            time: "3h",
            unread: false,
        },
        {
            ...
            icon: "truck",
            tone: "info",
            title: "Truck MXL-502 returned to base",
        ...
        },
        {
            ...
            icon: "box",
            tone: "success",
            title: "Container CNT-4492 loaded",
            sub: "Bologna Hub · 18.4 tons",
            time: "7h",
            unread: false,
        },
    ];
    ...
    const messages = [
        {
            ...
            name: "Luca Romano",
            preview: "Traffic cleared — I'm back on schedule for Turin.",
            time: "4m",
            unread: true,
        },
        {
            ...
            name: "Sofia Rinaldi",
            preview: "Need a swap for tomorrow's Rome run, possible?",
            time: "1h",
            unread: true,
        },
        {
            ...
            preview: "Crossed the border — container seal intact.",
            time: "3h",
            unread: false,
        },
        {
            ...
            preview: "Thanks for approving the reroute ✌️",
            time: "Yesterday",
            unread: false,
        },
        {
            ...
            name: "Davide Ferrari",
            preview: "Geneva depot received the shipment — paperwork signed.",
            time: "Yesterday",
            unread: false,
        },
    ];
    ...
    document.addEventListener("click", (e) => {
        const pop = $("#msgPop");
        if (!pop?.classList.contains("open")) return;
        if (!e.target.closest("#msgPop") && !e.target.closest("#msgBtn"))
    ...
    $("#AlertBtn")?.addEventListener("click", (e) => {
        ...
        $("#notifyPop")?.classList.remove("open");
        $("#msgPop")?.classList.remove("open");
        $("#AlertPop")?.classList.toggle("open");
    });
    document.addEventListener("click", (e) => {
        const pop = $("#AlertPop");
        if (!pop?.classList.contains("open")) return;
        if (!e.target.closest("#AlertPop") && !e.target.closest("#AlertBtn"))
            pop.classList.remove("open");
    });
    $("#AlertMarkAll")?.addEventListener("click", (e) => {
        e.preventDefault();
        e.stopPropagation();
        messages.forEach((m) => (m.unread = false));
        renderMessages();
    ...
    $("#AlertCompose")?.addEventListener("click", (e) => {
        e.stopPropagation();
        $("#AlertPop").classList.remove("open");
        toast("New alert composer — coming soon");
    });
    ...
    document.addEventListener("click", (e) => {
        ...
        if (mini) {
            ...
            if (mini.classList.contains("mini-btn--danger")) {
                if (
                    confirm(
                        "Delete this record? This can be undone within 30 days.",
                    )
                )
            ...
            if (title === "View") {
                ...
                const idCell = row?.querySelector(
                    isDrivers ? ".asset-sub" : ".asset-name",
                );
                const id = idCell?.textContent.trim();
                if (!id) {
                    toast("No record ID found");
                    return;
                }
                if (bc.contains("page-trucks")) {
                    location.href = `truck-detail.html?id=${encodeURIComponent(id)}`;
                    return;
                }
                if (bc.contains("page-containers")) {
                    location.href = `container-detail.html?id=${encodeURIComponent(id)}`;
                    return;
                }
                if (isDrivers) {
                    location.href = `driver-detail.html?id=${encodeURIComponent(id)}`;
                    return;
                }
                if (bc.contains("page-trips")) {
                    location.href = `trip-detail.html?id=${encodeURIComponent(id)}`;
                    return;
                }
            ...
            if (title === "Edit") {
                ...
                const id = idCell?.textContent.trim();
                if (id) {
                    if (bc.contains("page-trucks") && window.Flecso.editTruck) {
                        window.Flecso.editTruck(id);
                        return;
                    }
                    if (
                        bc.contains("page-containers") &&
                        window.Flecso.editContainer
                    ) {
                        window.Flecso.editContainer(id);
                        return;
                    }
                    if (isDrivers && window.Flecso.editDriver) {
                        window.Flecso.editDriver(id);
                        return;
                    }
                    if (bc.contains("page-trips") && window.Flecso.editTrip) {
                        window.Flecso.editTrip(id);
                        return;
                    }
                }
        ...
        if (!hiddenFileInput) {
            ...
            hiddenFileInput.addEventListener("change", () => {
                if (hiddenFileInput.files.length)
                    toast(`Uploaded: ${hiddenFileInput.files[0].name}`);
    ...
    document.addEventListener("click", (e) => {
        ...
        if (btn.closest(".modal__footer") || btn.closest(".drawer__footer"))
            return;

        const label = btn.textContent.trim();

        if (btn.closest(".upgrade-card")) {
            location.href = "settings.html#subscription";
            return;
        }
        if (btn.classList.contains("chip")) {
            toast("All services operational · 99.98% uptime");
            return;
        }
        if (btn.closest(".sos-item")) {
            toast("Emergency team dispatched to location");
            return;
        }
        ...
        if (btn.closest(".plan")) {
            ...
            toast(
                label === "Current Plan"
                    ? `${plan} is your current plan`
                    : `Upgrade to ${plan} — redirecting to checkout…`,
            );
            return;
        }
        ...
        if (btn.closest(".tab-panel[data-panel='legal']")) {
            const t =
                btn.closest("div").querySelector("h5")?.textContent ||
                "document";
        ...
        if (label === "New Role" || label.includes("New Role")) {
            toast("Role creation wizard — coming soon");
            return;
        }
        ...
        if (btn.closest(".page-head__actions") || btn.closest(".card__head")) {
            if (
                label.includes("Last 30") ||
                label.match(
                    /Last \d+ days|Today|Yesterday|This week|This month/,
                )
            ) {
                openDateRangeMenu(btn);
                return;
            }
            if (label.includes("Quick add")) {
                openQuickAddMenu(btn);
                return;
            }
            if (label.includes("Alert settings")) {
                location.href = "settings.html#notifications";
                return;
            }
            ...
            if (label.includes("Schedule trip")) {
            ...
            if (
                ...
                toast("Opening full list…");
                return;
            }
            if (label.includes("Filters")) {
                toast("Advanced filters panel — coming soon");
                return;
            }
            if (label.includes("Log Service")) {
                toast("Service log entry form — coming soon");
                return;
            }
        }
        ...
        if (
            btn.closest(".detail-hero__actions") ||
            btn.closest(".side-action") ||
            btn.classList.contains("side-action")
        ) {
            // Assign / Reassign flows → go to trips page and open the New Trip drawer
            if (
                /Assign (to )?Trip|Assign to a trip|Reassign trip/i.test(label)
            ) {
                location.href = "trips.html?new=1";
                return;
            }
            // Trip-specific
            if (
                label.includes("Notify Driver") ||
                label.includes("Notify driver")
            ) {
                toast("Driver notified · push alert sent");
                return;
            }
            if (label.includes("Reroute")) {
                toast("Reroute planner — coming soon");
                return;
            }
            if (label.includes("Cancel trip")) {
                if (
                    confirm(
                        "Cancel this trip? Driver and customer will be notified.",
                    )
                )
                    toast("Trip cancelled · stakeholders notified");
                return;
            }
            if (label.includes("Download BoL") || label.includes("Print BoL")) {
                toast("Generating Bill of Lading PDF…");
                return;
            }
            // Truck / Container
            if (
                label.includes("Schedule service") ||
                label.includes("Schedule inspection")
            ) {
                toast("Maintenance calendar opened");
                return;
            }
            if (label.includes("Archive")) {
                if (
                    confirm(
                        "Archive this record? It will be hidden from active lists.",
                    )
                )
                    toast("Record archived");
                return;
            }
            if (label.includes("Deactivate")) {
                if (
                    confirm(
                        "Deactivate this driver? They won't be assignable to trips.",
                    )
                )
            ...
            // Driver
            if (label.includes("Send a message") || label === "Message") {
                toast(
                    `Opening conversation with ${document.querySelector(".detail-hero h1")?.textContent || "driver"}…`,
                );
                return;
            }
            if (label.includes("Generate timesheet")) {
                toast("Timesheet PDF generated");
                return;
            }
            if (label.includes("Generate report")) {
                toast("Report PDF generated");
                return;
            }
            if (
                label.includes("Upload document") ||
                label.includes("Upload documents") ||
                label.includes("Attach document")
            ) {
                triggerFilePicker();
                return;
            }
            ...
            if (label === "Edit") {
                const bc = document.body.classList;
                const heroTitle =
                    document.querySelector(".detail-hero h1")?.textContent ||
                    "record";
                if (bc.contains("page-trucks")) {
                    location.href = `trucks.html?edit=${encodeURIComponent(heroTitle)}`;
                    return;
                }
                if (bc.contains("page-containers")) {
                    location.href = `containers.html?edit=${encodeURIComponent(heroTitle)}`;
                    return;
                }
                if (bc.contains("page-drivers")) {
                    location.href = `drivers.html?edit=${encodeURIComponent(heroTitle)}`;
                    return;
                }
                if (bc.contains("page-trips")) {
                    location.href = `trips.html?edit=${encodeURIComponent(heroTitle)}`;
                    return;
                }
                toast("Opening editor…");
                return;
    ...
    function openDateRangeMenu(anchor) {
        const ranges = [
            ...
            { label: "Last 7 days", value: "7d" },
            {
                label: "Last 30 days",
                value: "30d",
                active: anchor.textContent.includes("30"),
            },
        ...
        openMenu(anchor, ranges, (value, label) => {
            if (value === "custom") {
                toast("Custom date range picker — coming soon");
                return;
            }
    ...
    function openQuickAddMenu(anchor) {
        ...
        openMenu(anchor, items, (value) => {
            ...
            if (routes[value]) location.href = routes[value];
                ...
                triggerFilePicker();
                toast("Select a CSV file to import");
            }
        });
    }
    ...
    function exportCsv() {
        ...
        if (document.body.classList.contains("page-trucks") && D.trucks) {
            ...
            document.body.classList.contains("page-containers") &&
            D.containers
        ) {
            rows = [
                [
            ...
            document.body.classList.contains("page-drivers") &&
            D.drivers
        ) {
            rows = [
                [
            ...
            document.body.classList.contains("page-sos") &&
            D.sosAlerts
        ) {
            rows = [
                [
    ...
    function setField(label, value) {
        ...
        for (const l of labels) {
            ...
            if (txt === label || txt.startsWith(label)) {
                const input = l.parentElement.querySelector(
    ...
    if (/[?&]new=1/.test(location.search)) {
        const ids = {
            "page-trucks": "addTruckBtn",
            "page-containers": "addContainerBtn",
            "page-drivers": "addDriverBtn",
            "page-trips": "addTripBtn",
        };
        const cls = Object.keys(ids).find((c) =>
            document.body.classList.contains(c),
        );
        if (cls)
            setTimeout(() => document.getElementById(ids[cls])?.click(), 120);
    }
})();
