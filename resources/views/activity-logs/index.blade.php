...
@media(max-width:500px){ .al-stats{ grid-template-columns:1fr; } }
...
.al-stat-top { display:flex; align-items:flex-start; justify-content:space-between; }
.al-stat-icon {
...
.al-stat-bottom { display:flex; align-items:center; justify-content:space-between; margin-top:4px; }
.al-stat-trend {
    display:inline-flex; align-items:center; gap:4px;
    font-size:11px; font-weight:600; border-radius:20px;
    padding:2px 8px;
}
.al-stat-trend.up   { background:#e6f9ee; color:#1a7a35; }
.al-stat-trend.down { background:#fdecea; color:#c0392b; }
.al-stat-trend.neu  { background:var(--ink-50); color:var(--ink-500); }
.al-spark { display:block; }
...
.al-filter-left  { display:flex; align-items:center; gap:10px; flex-wrap:wrap; flex:1; }
.al-filter-right { display:flex; align-items:center; gap:8px; flex-shrink:0; }

.al-search-wrap { position:relative; min-width:240px; }
.al-search-wrap svg {
    position:absolute; left:10px; top:50%;
    transform:translateY(-50%); pointer-events:none;
    color:var(--ink-400);
}
.al-search-wrap .input { padding-left:32px; height:34px; font-size:13px; }
...
.al-tab:hover { background:var(--ink-50); color:var(--ink-800); }
.al-tab.active {
    background:var(--ink-900); color:#fff;
    border-color:var(--ink-900);
}
.al-tab.post.active   { background:#1a7a35; border-color:#1a7a35; }
.al-tab.put.active    { background:#b36200; border-color:#b36200; }
.al-tab.delete.active { background:#c0392b; border-color:#c0392b; }
...
.al-log-content { min-width:0; flex:1; }
.al-log-sentence {
    font-size:15px;
    font-weight:600;
    color:var(--ink-900);
...
.al-method.post   { background:#e6f9ee; color:#1a7a35; }
.al-method.put    { background:#fff4e0; color:#b36200; }
.al-method.delete { background:#fdecea; color:#c0392b; }
.al-method.visit  { background:var(--ink-100); color:var(--ink-600); }

.al-empty, .al-client-empty {
    text-align:center;
    padding:60px 20px;
    color:var(--ink-400);
}
.al-empty svg, .al-client-empty svg {
    opacity:.2;
    margin-bottom:12px;
    display:block;
    margin-inline:auto;
}
.al-client-empty { display:none; }

.al-pagination { padding:12px 16px; border-top:1px solid var(--surface-line); margin-top:14px; }
.al-pagination nav { display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:10px; }
.al-pagination nav > div:first-child { font-size:12px; color:var(--ink-400); }
.al-pagination nav > div:last-child  { display:flex; align-items:center; gap:4px; }
.al-pagination span[aria-current="page"] > span,
.al-pagination a {
...
.al-pagination a:hover { background:var(--ink-50); border-color:var(--ink-300); color:var(--ink-900); }
.al-pagination span[aria-current="page"] > span { background:var(--ink-900); color:#fff; border-color:var(--ink-900); }
.al-pagination span:not([aria-current]) > span {
    display:inline-flex; align-items:center; justify-content:center;
    min-width:32px; height:32px; padding:0 10px; border-radius:7px;
    font-size:13px; border:1px solid var(--surface-line);
    background:white; color:var(--ink-300); cursor:not-allowed;
}
.al-pagination svg { display:none; }
.al-pagination [rel="prev"]::before { content:"←"; }
.al-pagination [rel="next"]::after  { content:"→"; }
</style>
    ...
    <div class="al-stats">
        <div class="al-stat-card">
            ...
            <div class="al-stat-bottom">
                ...
                <svg class="al-spark" viewBox="0 0 80 28" width="80" height="28" fill="none">
                    <polyline points="0,24 14,18 28,20 42,12 56,15 70,8 80,10"
                        stroke="var(--ink-400)" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
            </div>
        </div>
        ...
        <div class="al-stat-card">
            ...
            <div class="al-stat-bottom">
                ...
                <svg class="al-spark" viewBox="0 0 80 28" width="80" height="28" fill="none">
                    <polyline points="0,22 14,16 28,18 42,10 56,14 70,6 80,9"
                        stroke="#1a7a35" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
            </div>
        </div>
        ...
        <div class="al-stat-card">
            ...
            <div class="al-stat-bottom">
                ...
                <svg class="al-spark" viewBox="0 0 80 28" width="80" height="28" fill="none">
                    <polyline points="0,20 14,22 28,14 42,16 56,10 70,12 80,7"
                        stroke="#1a7a35" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
            </div>
        </div>
        ...
        <div class="al-stat-card">
            ...
            <div class="al-stat-bottom">
                ...
                <svg class="al-spark" viewBox="0 0 80 28" width="80" height="28" fill="none">
                    <polyline points="0,8 14,12 28,10 42,16 56,14 70,20 80,22"
        ...
        <div class="al-filter-left">
            <div class="al-search-wrap">
                ...
                <input
                    ...
                    style="height:34px;font-size:13px;min-width:240px">
            </div>

            <div class="al-tabs" id="methodTabs">
                <button type="button" class="al-tab {{ !request('method_filter') ? 'active' : '' }}" data-method="">All</button>
                <button type="button" class="al-tab post {{ request('method_filter') === 'POST' ? 'active' : '' }}" data-method="post">POST</button>
                <button type="button" class="al-tab put {{ request('method_filter') === 'PUT' ? 'active' : '' }}" data-method="put">PUT</button>
                <button type="button" class="al-tab delete {{ request('method_filter') === 'DELETE' ? 'active' : '' }}" data-method="delete">DELETE</button>
            </div>
        </div>
    ...
    <div class="al-feed" id="activityFeed">
        ...
        @else
            @foreach($logs as $log)
                ...
                <article
                    class="al-log-card"
                    data-user="{{ strtolower($name) }}"
                    data-route="{{ strtolower($log->route ?? '') }}"
                    data-action="{{ strtolower($log->action ?? '') }}"
                    data-method="{{ $badgeMethod }}"
                    data-date="{{ optional($log->created_at)->format('Y-m-d') }}"
